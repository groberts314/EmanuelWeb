...
- **No package manager, no lint/test/build commands.** Edits to `.php`/`.phtml` are live the moment they hit the server. Validate by reading carefully and, if needed, running a local PHP server (`php -S localhost:8000`) from the repo root — but the `.htaccess` rewrites won't apply under the built-in server, so friendly URLs need to be hit as `whatever.php?year=...&month=...` directly.
